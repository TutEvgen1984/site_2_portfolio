...
/**
 * Удаляет/добавляет ограничение 1920px содержимому на странице
 * @param {'add' | 'remove' | string} action_name параметр может быть либо 'remove' либо 'add'
 */
...
// Сразу после загрузки страницы проверяем была ли нажата кнопка была пользователем в прошлой сессии, если да, то необходимо удалить ограничивающие классы:
if (localStorage.getItem('zoom-out--pressed') == "true") { // с помощью нестрогого равенства проверяем значение на true (обязательно писать в кавычках, без кавычек работать не будет потому что в localStorage оно хранится в виде строки, а не в виде булева)
    // убираем ограничивающие классы на странице
    w1920 ('remove');
}
